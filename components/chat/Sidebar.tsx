...
import {
  Search, Plus, MessageSquare, Users, X, Check, Trash2,
} from "lucide-react";
import { formatDistanceToNow } from "date-fns";

const isUserOnline = (user: any) => {
  if (!user?.isOnline) return false;
  return Date.now() - (user.lastSeen ?? 0) < 25000;
};
      ...
      <div className="p-4 border-b border-white/5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {/* ✅ Added sidebar-new-chat-btn class */}
            <button
              onClick={() => setShowNewChat(true)}
              className="sidebar-new-chat-btn w-8 h-8 rounded-lg bg-[#22d3a0]/10 hover:bg-[#22d3a0]/20 text-[#22d3a0] flex items-center justify-center transition"
            >
              <Plus size={16} />
            </button>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filteredConversations?.map((conv: any) => (
          <div
            ...
            className={`conversation-item group flex items-center gap-3 px-4 py-3 cursor-pointer transition border-l-2 ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                {conv.lastMessage && (
                  <span className="text-xs text-slate-600 flex-shrink-0 ml-2 group-hover:hidden">
                    {formatDistanceToNow(conv.lastMessage._creationTime, { addSuffix: false })}
                  </span>
                )}
              </div>
              {conv.lastMessage && (
                <p className={`text-xs truncate mt-0.5 ${
                  conv.lastMessage.isDeleted ? "italic text-slate-600" : "text-slate-500"
                }`}>
                  {conv.lastMessage.isDeleted
                    ...
                    ? "📄 Document"
                    : conv.lastMessage.content.startsWith("https://")
                    ? "📎 Attachment"
            ...
            <button
              ...
              className="opacity-0 group-hover:opacity-100 flex-shrink-0 w-7 h-7 rounded-lg bg-red-500/10 hover:bg-red-500/25 text-red-400 flex items-center justify-center transition"
            >
              {deletingId === conv._id ? (
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#111827] border border-white/10 rounded-2xl w-96 max-h-[80vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-white/5">
              ...
              <button
                onClick={() => { setShowNewChat(false); setSelectedUsers([]); setUserSearch(""); setGroupName(""); setIsGroup(false); }}
              ...
              <div className="flex items-center gap-2 p-1 bg-white/5 rounded-lg">
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-md text-sm font-medium transition ${
                    !isGroup ? "bg-[#22d3a0] text-black" : "text-slate-400 hover:text-white"
                  }`}
                >
                  Direct Message
                </button>
            ...
            {isGroup && selectedUsers.length > 0 && (
              <div className="px-4 py-2 flex flex-wrap gap-1">
                {selectedUsers.map((id) => {
                  ...
                  return (
                    <span key={id} className="flex items-center gap-1 px-2 py-1 bg-[#22d3a0]/10 text-[#22d3a0] text-xs rounded-full">
                      {u?.name}
                      <button onClick={() => toggleUserSelection(id)}><X size={10} /></button>
                    </span>
                  );
                })}
              </div>
